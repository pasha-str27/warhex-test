...
namespace Assets.TestScripts.triangulation
{
    public class Line :IComparable<Line>, IEquatable<Line>
    {
        ...
        public bool isPointOnLine(Vector2 point,float delta)
        {
            ...
            //print(Origin);
            //print(Destination);
            //if (!((point.x >= Origin.x-delta && point.x <= Destination.x+delta) && (point.y >= Origin.y-delta && point.y <= Destination.y+delta)))
            //&&!(point.x >= Destination.x && point.x <= Origin.x) && (point.y >= Destination.y && point.y <= Origin.y)))
            //|| (point.x >= Destination.x && point.x <= Origin.x) && (point.y >= Destination.y && point.y <= Origin.y)))
            //if (!((point.x >= Origin.x-delta && point.x <= Destination.x+delta) && (point.y <= Origin.y-delta && point.y >= Destination.y+delta)))
            //    return false;
            //return false;
            ...
            //if(point.y < Origin.y || point.y > Destination.y)
            //    return false;
